...
class LitModel(pl.LightningModule):
    ...
    def sample(self, N, device, T=None, T_latent=None):
        if T is None:
            ...
            latent_sampler = (
                self.conf._make_latent_diffusion_conf(T_latent).make_sampler()
            )
    ...
    def is_last_accum(self, batch_idx):
        ...
        used with gradient_accum > 1 and to see if the optimizer
        will perform "step" in this iteration or not.
        """
    ...
    def infer_whole_dataset(self,
        ...
        Args:
            both_flips: include both original and flipped images;
                no need, it's not an improvement
            with_render: whether to also render the images corresponding
                to that latent
    ...
    def training_step(self, batch, batch_idx):
        ...
        with amp.autocast(False):
            ...
            if self.conf.train_mode == TrainMode.diffusion:
                ...
                main training mode!!!
                """
                # with numpy seed we have the problem
                # that the sample t's are related!
            ...
            with torch.no_grad():
                ...
                for x_T in loader:
                    ...
                    if self.conf.train_mode.is_latent_diffusion(
                        ...
                        if (
                            not use_xstart and
                            self.conf.model_type.has_noise_to_cond()
                        ):
                            model: BeatGANsAutoencModel
                            # special case, it may not be stochastic,
                            # yet can sample
    ...
    def evaluate_scores(self):
        ...
        if (
            self.conf.eval_every_samples > 0 and
            self.num_samples > 0 and
            is_time(
        ...
        if (
            self.conf.eval_ema_every_samples > 0 and
            self.num_samples > 0
            and is_time(
    ...
    def test_step(self, batch, *args, **kwargs):
        ...
        """
        "infer" = predict the latent variables using
            the encoder on the whole dataset
        """
        ...
        """
        "infer+render" = predict the latent variables using
            the encoder on the whole dataset
            ...
            if each.startswith('infer+render'):
                ...
                if m is not None:
                    ...
                    conds = self.infer_whole_dataset(
                        with_render=True,
                        T_render=T,
                        render_save_path=render_save_path_
                    )
        ...
        "fid<T>,<T_latent>" = unconditional generation for latent models
            (conf.train_mode = latent_diffusion).
            Note:   Diff. autoenc will still NOT receive real images in
                    this made. But you need to make sure that the train_mode
                    is latent_diffusion.
        """
        for each in self.conf.eval_programs:
            if each.startswith('fid'):
                ...
                if m is not None:
                    ...
                    if m is not None:
                        ...
                        msg_ = f"evaluating FID (clip latent noise) T = {T}..."
                        msg_ += f" latent T = {T_latent}"
                        print(msg_)
                    else:
                        # evalT
                        _, T = each.split('fid')
        ...
        for each in self.conf.eval_programs:
            if each.startswith('inv'):
                ...
                _, T = each.split('inv')
                T = int(T)
                msg_ = "evaluating reconstruction with noise inversion "
                msg_ += f"T = {T}..."
                print(msg_)
