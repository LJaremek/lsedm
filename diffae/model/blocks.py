...
from .nn_custom import (
    avg_pool_nd, conv_nd, linear, normalization, torch_checkpoint, zero_module
    )
...
class ResBlock(TimestepBlock):
    ...
    def __init__(self, conf: ResBlockConfig):
        ...
        if conf.out_channels == conf.channels:
            # cannot be used with gatedconv, also gatedconv is alsways
            # used as the first block
...
def apply_conditions(
    ...
    # default, the scale & shift are applied after
    # the group norm but BEFORE SiLU
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        if num_head_channels == -1:
            ...
            msg = f"q,k,v channels {channels} is not divisible by "
            msg += f"num_head_channels {num_head_channels}"
            assert (
