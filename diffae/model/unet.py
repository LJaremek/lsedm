...
from .blocks import (
    ...
    ResBlockConfig, AttentionBlock,
    Downsample, Upsample
)
from .nn_custom import (
    conv_nd, linear, normalization, timestep_embedding, zero_module
    )
...
class BeatGANsUNetConfig(BaseConfig):
    ...
    # you can also set the number of resblocks specifically
...
class BeatGANsUNetModel(nn.Module):
    def __init__(self, conf: BeatGANsUNetConfig):
        ...
        self.time_emb_channels = (
            conf.time_embed_channels or conf.model_channels
        )
        ...
        for level, mult in list(enumerate(conf.channel_mult))[::-1]:
            for i in range(conf.num_res_blocks + 1):
                ...
                try:
                    ...
                    # we will not have enough lateral (skip) connecions
                    # for all decoder blocks
                    ich = 0
                # print('pop:', ich)
                layers = [
                    ResBlockConfig(
