...
from model import Model, BeatGANsAutoencModel
...
from dist_utils import (
    barrier, get_world_size, all_gather, get_rank, chunk_size, broadcast
)
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(val_loader, desc='lpips'):
            ...
            if conf.model_type == ModelType.ddpm:
                # the case where you want to calculate the
                # inversion capability of the DDIM model
    ...
    with torch.no_grad():
        if conf.model_type.can_sample():
            ...
            if conf.train_mode.is_latent_diffusion():
                # evaluate autoencoder + latent diffusion
                # (doesn't give the images)
                ...
                # to make the FID fair, autoencoder must not see the validation
                # dataset also shuffle to make it closer to unconditional
                # generation
